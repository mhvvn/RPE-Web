...
const ArticleManager: React.FC = () => {
    ...
    try {
      for (let i = 0; i < files.length; i++) {
        ...
        if (type === 'image' && !currentPost.image_url) {
            setCurrentPost(prev => ({ ...prev, image_url: base64Url }));
        }
      }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (currentPost.id) {
      // Update existing
      updateNews(postData as NewsItem);
    } else {
      // Create new
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
              ...
              <div>
                ...
                <div className="flex flex-col space-y-2">
                    ...
                    <div className="flex">
                        ...
                        className="flex-1 px-4 py-2 rounded-r-lg border border-slate-300 focus:ring-2 focus:ring-sea-500 outline-none"
                        />
                    </div>
                </div>
              </div>
          ...
          <div className="pt-6 border-t border-slate-200">
             ...
             <div className="grid grid-cols-1 gap-6">
                <div>
                    <label className="block text-sm font-medium text-slate-700 mb-1">URL Galeri Foto Eksternal (Satu URL per baris)</label>
                    <textarea 
                        rows={3}
                        value={galleryText}
