...
const UserManager: React.FC = () => {
  ...
  const handleCreate = () => {
    setCurrentEditUser({ ...initialUser, id: Date.now().toString() });
    setIsEditing(true);
  };
  ...
  const handleDelete = (id: string) => {
    ...
    // PROTECTION LOGIC
    // Cast to Role to prevent TS error about non-overlapping types if inferred narrowly
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (users.some(u => u.id === userData.id)) {
      // Logic for keeping old password if not changed in edit mode
      // This is handled by state because currentEditUser is populated with full object including old password on Edit
      updateUser(userData);
    } else {
      addUser(userData);
    }
                ...
                <tr key={user.id} className="hover:bg-slate-50 transition">
                  ...
                  <td className="px-6 py-4">
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold uppercase ${
                          user.role === Role.SUPER_ADMIN ? 'bg-slate-800 text-white' :
